...
**2계열 병렬 검수 = Codex(GPT) + Opus(Read).** 같은 family는 오류가 상관됨 → 한쪽이 놓친 해부학·종/색 드리프트를 다른 family가 잡는다. 100권 7웨이브 Opus 검수에도 펭귄 3물갈퀴·흰→노랑 고양이가 통과된 실사고(2026-06-13 사용자 적발)가 근거.
1. **MD5 기계스캔 먼저**: `node scripts/pipeline-gates.mjs dups` — 책 내/책 간 바이트 동일 페이지 적발(50권 중 6쌍 실적발). 시각 에이전트는 "느슨한 중복"을 놓침.
2. **🤝 Codex 비전 교차검증 (전수 스캐너 — 신설)**: `node scripts/codex-vision-qa.mjs all --conc 3 --effort medium`. 컷당 1콜로 **해부학(사람 손가락·팔 + 동물 종별 다리 수·새/펭귄 날개·물갈퀴) + 종/색 드리프트(표지 앵커 `-i` + 정본 char 텍스트 대비)** 동시 검사. resumable 리포트 `scripts/.vision-qa-report.json`. medium effort 컷당 ~6s(conc3). HIGH = 재생성 후보. (ChatGPT Pro라 한도 무관, Claude 토큰 안 씀)
3. **에이전트 N개 병렬 육안 검수(Opus)**: 각자 4~5권씩 webp Read로 그림↔문장 대조 + Codex HIGH 컷 재확인. 둘 다 HIGH = 고신뢰 / 불일치 = Opus 직접 판정.
4. **불일치 컷 격리 재생성**: `regen-fixes.mjs`·`regen-dedup-w7.mjs` 패턴 — **conc=1 단독** + `[unique:<salt>]` + 강한 NEGATIVE(`a penguin has exactly 2 flippers`, `the cat stays WHITE not yellow` 등 구체) + cover 앵커 + 생성 직후 md5 self-verify. 재생성분도 Read 육안 재검 후 cwebp 교체 → `pipeline-gates.mjs dups` 재실행.
5. 그림↔문장 매핑(page i→pN.webp)은 코드상 1:1 정상 — "불일치"는 생성 drift이므로 선별 재생성만.
...
| `scripts/codex-vision-qa.mjs` | codex **이미지** 교차검수 — 해부학(사람+동물)·종/색 드리프트, 전수/특정책, resumable |
